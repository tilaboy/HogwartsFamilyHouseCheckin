...
顶栏把「我是谁」和两个动作拆成了三块：身份牌（只读显示名字 · 学院）、**换人**、**分院**。
换人只重选身份、直接进入，适合姐姐用完递给弟弟；分院才会走选学院那一步，改学院不影响已得分。

### 阶梯计分
需要累计时长的任务按时长给分，而不是固定值。中文阅读、英语练习、单簧管/架子鼓练习用三档：
...
Scholastic 阅读用两档（保留原有 15 分钟 = 10 分的价值）：`15′+10 / 30′+20`。

档位由任务的 `tiers` 字段驱动（`[[10,5],[20,10],[30,20]]`），前端按钮与后端结算读同一份配置，
所以家长改档位不用改代码。每一行的按钮都长在任务行上，没有单独的「今日阅读」卡片。
